...
async function enviarDatos(asistencia) {
    ...
    const mensaje = document.getElementById('customMessage').value.trim();
    ...
    const data = { nombre, apellido, asistencia, mensaje };
    const url = "https://script.google.com/macros/s/AKfycbxhgCyhsbaFTxnQDJBMGjMzmZq7Bm8S0efpmJCy9Jmsu_zjebg83pWVBoyxoDNQJNkJ6A/exec"; // <-- con /exec

    try {
        await fetch(url, {
            method: "POST",
            mode: "no-cors",  // evita el error CORS
...
document.getElementById("botonSi").addEventListener("click", () => enviarDatos("Si"));
document.getElementById("botonNo").addEventListener("click", () => enviarDatos("No"));